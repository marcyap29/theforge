...
- **Architect epics + build gate (v0.4.40).** Features carry a `buildKind` (`BuildKind`: standard | epic | manual) + optional `parentId` (drift schemaVersion 3→4, additive migration defaulting `standard`). `FeatureScanner.architectFeature` returns a JSON `ArchitectPlan` (isEpic + typed `ArchitectSubFeature`s); `showArchitectReviewSheet` lets the user keep a subset; applying marks the parent `epic` and creates the sub-features nested via `parentId`, each tagged standard/manual. `_buildFeature` gates on `buildKind.isDirectlyBuildable` — an epic/manual item routes to Architect / How-to-build (with an explicit override) instead of being silently stubbed. This is the structural fix for "single-feature executor stubs an epic and marks it shipped."
- **Build advice (v0.4.39).** `FeatureScanner.adviseBuild` gives *scale-aware* per-feature guidance (same repo/doc/code context): the output leads with a **scope** check (single feature vs epic) and a **feasibility** verdict (can Build-with-AI code-gen it, or does it need human/ML/dataset work), then approach + named packages, effort, risks, a sub-feature breakdown, and a suggested descriptor. Reached from the feature ⋮ menu → `BuildAdviceScreen` (on-demand, Regenerate/Copy, no disk cache — it's a deliberate question). Resolves the "unit of advice ≠ always a feature" problem: an epic like on-device recognition is flagged as such rather than hidden behind a short description.
...
1. **Scout** — pass one reads the repo (`gatherKeyDocs` + file gather), the feature's tracker context, the project's **ingested reference context** (`.forge/ingested/reference_context.md`), and **prior-build memory** (`.forge/build_memory/`, records of what earlier features shipped) — all re-read each round by `_plan`. It also chooses which **code files** and, for doc pools that overflow their always-on cap, which **doc-pool entries** to pull in full: `gatherDocManifest` offers a compact `{id,kind,title,preview}` manifest, the scout returns `{"files":[…],"docs":[…]}`, and `readDocEntry` (sandboxed) fetches the selected entries into a `## Retrieved reference material` block (de-duped against always-on context). The agent narrates its understanding (streamed as `thinking` / `presentation` lines).
...
- **Auto-boot (v0.4.37).** Discovery merges three sources: live targets (`flutter devices`), shut-down iOS simulators (`simctl list devices available --json` → `parseIosSimulators`, excluding UDIDs already live), and Android AVDs (`flutter emulators --machine` → `parseAndroidEmulators`). Bootable entries carry `needsBoot` + `BootMethod`; `start()` boots them first (`simctl boot` then poll for `Booted`, or `flutter emulators --launch` then poll `flutter devices` for the new android id) before `flutter run`. Pure parsers are unit-tested.
- **iOS deployment target (v0.4.37).** `lib/data/filesystem/ios_deployment.dart` holds pure, tested transforms — `bumpPbxproj` (raise `IPHONEOS_DEPLOYMENT_TARGET` below 15.0, never downgrade) and `bumpPodfile` (uncomment/raise the platform line + inject a `post_install` override forcing every pod to 15.0) — plus `applyMinIosDeploymentTarget(repoPath)`. Called after every `flutter create` (`createCodeRepo`, `scaffoldFlutter`) and as a pre-run safety net for iOS targets in `RunController.start`. Prevents the Xcode-27 "deployment target below 15.0" wall on new apps.
