...
| Pre-build guards (leftover work + already-built) | Shipped v0.4.58 | Before dispatching a new Build-with-AI run, `_preBuildChecks` runs two deterministic checks (no AI call): (1) `gitHasChanges` — warn if the repo has uncommitted changes from a prior/failed build; (2) `hasBuildMemory` — warn if the feature was built before (build record) but isn't marked shipped. Dismissable (Cancel / Build anyway); only gates a new build, never a re-attach. Stops the recurring duplicate-creation pattern (duplicate `_HighlightPainter`, `procedure_model.dart`) |
