...
| Architect epics + build gate | Shipped v0.4.40 | Feature `buildKind` (standard/epic/manual) + `parentId` (drift v4). `FeatureScanner.architectFeature` decomposes an epic into ordered typed sub-features → `showArchitectReviewSheet` → creates them nested under the epic. Build-with-AI is gated on epic/manual items (steers to Architect / How-to-build, explicit override). Board tags epic/manual. Stops big features being stubbed and marked shipped |
