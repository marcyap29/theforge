...
| BUG-IMPL-009 | IMPL | Reopening a feature greyed out repo actions (Make runnable / Commit & push) — re-attach path built `ImplBrief` with `repoPath: ''`; now reads linked repoPath from config | 2026-09-15 |
| BUG-IMPL-010 | IMPL | Shipping features back-to-back made redundant duplicate-titled commits — `_documentAndCommit` always LLM-rewrote the whole `ARCHITECTURE.md` (non-deterministic), so a repeat ship churned a docs-only commit even with nothing new; now `gitHasChanges` gates the refresh+commit (checked before the churn) and `shipFeature` no-ops when already shipped | 2026-09-20 |
...
| BUG-TRACKER-002 | TRACKER | Remove-duplicates missed reworded dups — semantic LLM pass was swallowed silently + parser only accepted one JSON shape (bare array → empty, no error); now tolerant parse + retry + reports failure (diag.log) | 2026-09-16 |
| BUG-TRACKER-003 | TRACKER | Architect looped on a non-epic — flow ignored `plan.isEpic`, always marked the parent an epic + nested a clone; the build gate then steered it back to Architect forever. Now branches on `isEpic`: single feature is sharpened in place + demoted to buildable | 2026-09-20 |
| BUG-LLM-002 | LLM | Ollama Cloud 401 on builds — **not a Forge defect**: bad/expired API key (regenerating a clean one fixed it). Long to diagnose due to two traps: `/api/tags` is public (200 with no auth, false "key works"), and zsh `read -s -p` is a bash-ism that silently left the test key empty. Forge-side: friendlier 401 msg + "Test key" button backlogged (§LLMKEY) | 2026-09-25 |
