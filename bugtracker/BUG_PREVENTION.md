...
- **When an LLM classifier returns a judgment, BRANCH on it — don't run the one true path and discard the flag.** The architect returns `isEpic` (true/false); the flow ignored `isEpic:false` and always marked the parent an epic + nested a clone. Combined with a build gate that steers epics *back* to Architect, that misclassification became an infinite loop (BUG-TRACKER-003). Two rules: (1) act on every branch of an LLM decision, including the "nothing to do / already fine" branch; (2) any state a gate reads (`buildKind → epic`) must be **reversible by the same flow** — else a single misclassification traps the item forever. Here the non-epic branch *demotes* a mis-marked epic back to buildable.
- **An Ollama Cloud `401 Unauthorized` in The Forge is almost always the KEY, not the app — regenerate it first.** Keys can be issued bad (copy/format problems) or expire; re-entering the *same* bad key won't help. The Forge sends `Authorization: Bearer <key>` to `ollama.com/api/chat` and reads the key from the login-keychain item `forge_api_key_ollama` (trimmed on save), so a 401 means `ollama.com` rejected the value. When diagnosing (BUG-LLM-002): (1) **never test an Ollama key against `/api/tags` — it's PUBLIC and returns 200 with no auth**; test `/api/chat` (the endpoint that authenticates and that builds call). (2) Marc's shell is **zsh**: `read -s -p` is a bash-ism that fails (`no coprocess`) and leaves the var EMPTY — a silent bad test; use `read -rs "K?prompt"`. (3) Keys saved by an **ad-hoc-signed** local build (`deploy_macos.sh` default) aren't readable by the **Developer ID–signed** release DMG and vice-versa (keychain ACL is per code-signature) → null read → 401; deploy local builds with `FORGE_SIGN_IDENTITY="Developer ID Application: Orbital AI, LLC (87W73WQSPF)"` so both share one entry.
...
- **Prefer targeted find/replace edits over whole-file rewrites; always `dart analyze` before committing AI output.** A full-file rewrite forces the model to reproduce the entire file, so it can silently drop code it didn't mean to change (theforge once lost ~345 lines + a stray `-` and wouldn't compile) and it fails outright when the file was read truncated. The Build-with-AI agent now returns small `{find, replace}` hunks for existing files (full `content` only for new files), so untouched code is never re-emitted; unmatched hunks are skipped with a note. Even so, a hunk can land a stray brace or drop a symbol — so The Forge runs an **analyze-gate** (`_analyzeGate`: `flutter/dart analyze`) immediately after applying edits and folds any `error •` diagnostics into the run's failure report (→ canFix/"Fix it"), catching a non-compiling edit in the same run instead of letting it land silently. Still: never commit AI edits unreviewed. See BUG-IMPL-003 / BUG-IMPL-004 / v0.4.15.
...
- **Run drift/sqlite3 on the main isolate unless a workload truly needs a background one.** `drift_flutter`'s `driftDatabase()` opens on a background isolate; `sqlite3`'s function/collation destructors are `NativeCallable.isolateLocal` FFI callbacks, and tearing them down on that worker trips a Dart FFI assertion (`DLRT_GetFfiCallbackMetadata`) that `abort()`s the whole app on close (BUG-IMPL-008). For a small local index, open a main-isolate `NativeDatabase` (via `LazyDatabase`) against the same file — same data, no cross-isolate FFI teardown, no abort. See BUG-IMPL-008.
- **Never `catch (_) {}` a best-effort AI pass into silence, and tolerant-parse model JSON.** A silently-swallowed failure makes a feature lie about its results — the "Remove duplicates" broom reported "no duplicates" when its semantic pass had actually errored (BUG-TRACKER-002). Log the failure (`DiagLog`) and report whether the pass ran. And accept any reasonable JSON shape (object `{"groups":[…]}` OR a bare `[[…]]` array OR fenced) — requiring one exact shape means a JSON-clean model that answers differently yields an empty result with no error. Retry once before giving up.
- **For JSON-only passes on a reasoning model, force thinking OFF or you get EMPTY content.** `qwen3.5`/`glm` are reasoning models; with thinking ON and a modest token budget, the model spends the whole budget in its *thinking* channel and returns an empty `content` string — indistinguishable at the parser from "ignored JSON mode," and no tolerant parse can recover it (there's nothing to parse). Pass `think: false` (via the `think` override on `LlmService.complete`) for Scan / Recommend / Plan-build-order / Remove-duplicates. See BUG-TRACKER-002 part 2 (v0.4.34).
- **When a JSON answer is required, enforce it — don't just ask for it.** A model (especially with thinking disabled) may reason in prose in its answer channel instead of emitting the JSON object ("did not return valid JSON", raw shows musings + code fences). Send Ollama `format: "json"` (`jsonMode`) on any pass whose output you `jsonDecode` (scout, plan, scan, dedup) so the output is constrained to valid JSON. Keep asking for the schema in the prompt too. See v0.4.19.
- **A "did not return valid JSON" failure is often truncation, not malformed output.** A large multi-file plan (many code hunks) can overrun the output-token budget and be cut off mid-JSON. Give the plan pass a generous ceiling (`_planTokens` 32k), and on parse failure detect truncation (`_looksTruncated`: JSON started but never closed) — then retry asking for a *smaller* plan, since retrying the same size just truncates again. See BUG-IMPL-004 (v0.4.17).
- **Never claim context the message doesn't actually contain, and guard long model streams against repetition.** A prompt that asserts "the file contents are provided below" when none were included makes the model spiral trying to reconcile the gap (BUG-IMPL-007). Keep such claims conditional on what you actually appended. Separately, any streamed completion needs a degeneration guard: if a long recent passage (~500 chars) repeats verbatim, abort and fail fast instead of burning the whole token budget on a loop. See BUG-IMPL-007.
- **The canonical projects root holds *workspaces* (docs), never code repos.** A project's `.forge/` workspace under `~/Documents/The Forge Projects/` is deliverables only; a code repo must live elsewhere (e.g. `~/Development/<name>`). Validate any user-pickable repo path with `ProjectFileRepository.isInsideProjectsRoot` and reject it if inside the projects root — otherwise Build-with-AI writes generated code in among the specs/handoffs, invisible to anyone looking in the expected code folder. A mislink must be *recoverable* (relocate + move code), not permanent. See BUG-IMPL-006.
- **Any repeatable import (scan / check-in) must dedup against existing records before writing.** A stateless generator (the feature scanner) re-proposes its whole list every run, so importing without a dedup step doubles rows on the second scan. Filter proposals against a normalized key (`_normTitle`: lowercased, non-alphanumerics collapsed, trimmed) of what's already tracked — and within the batch itself — before adding. See BUG-TRACKER-001.
- **A reasoning model can spend a whole turn thinking and never emit the answer.** It may also try to "ask" for more files it can't be given. Give a generous explicit token ceiling (Ollama `num_predict = maxTokens`, plan pass ≥ 8000) so thinking doesn't starve the answer; instruct it to make its best plan with the files it has and that its entire final answer MUST be the JSON object; and **auto-retry once** (firmly demanding JSON only) before failing. Prefer an instruction-following/coder model for Build with AI. See BUG-IMPL-004.
- **A non-deterministic doc regeneration must never be the sole reason a commit exists.** The ship flow LLM-rewrites the whole `ARCHITECTURE.md`, which differs every run — so an auto-commit that always includes it will churn a spurious "docs-only" commit on every repeat ship (duplicate titles, empty of real change). Gate any auto-commit on a real working-tree change (`git status --porcelain` via `gitHasChanges`) checked **before** the non-deterministic step, and make idempotent actions like ship a no-op when there's nothing new (both an in-session flag and the durable tree check). See BUG-IMPL-010.
